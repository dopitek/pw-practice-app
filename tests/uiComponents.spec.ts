...
test('lists and dropdowns', async({page})=>{
    ...
    page.getByRole('list'); // when has ul tag
    page.getByRole('listitem') // when has li tag
...
test('web tables', async({page}) =>{
    ...
    await page.locator('input-editor').getByPlaceholder('E-mail').fill('newemail');

    await page.locator('.nb-checkmark').click()

    await expect(targetRowById.locator('td').nth(5)).toHaveText('newemail');
